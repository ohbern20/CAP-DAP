...
```
You might make a note here to see Data_Exploration.Rmd to see how the data were cleaned.
...
Decide on statistical test: I will use a linear regression since the predictor variable (canopy density) is somewhat continuous (0-100), and the response variable (regeneration) is also continuous. 
...
The normal Q-Q graph looks good, and indicates the data are fairly normally distributed, with some smaller and larger values deviating. The Residuals vs. fitted graph indicates relatively equal variance, but is a little off. I am comfortable moving forward for now, but I might change my statistical test to a GLM in the future.  
...
Based on the autoplot() results, I feel moderately comfortable moving ahead and assuming these data meet the linear model assumptions. I might change this test to a GLM as well as the assumptions could be better met. However, it is unlikely the outcome of the statistical test would be very different. 
...
Slope is estimated to be -5 and slope as 0.7, which is fairly similar to what I predicted. However, the R2 value is fairly small which means even though there is a significant relationship between canopy density and bare ground it is not a particularly strong relationship. 
...
ggplot(ferns, aes(x = Percent_ferns)) +
  ...
  theme_bw() 
```
It is possible I will run into an issue known as over dispersion that results from, in this case, the high number of instances with zero ferns. I have not yet worked out how to account for this, but might do so for my final analysis. 
...
I will fit a generalized linear model because I am dealing with a categorical predictor variable with more than two groups (Forest) and a continuous, yet bounded, (0-1) response variable (percent ferns). A generalized linear model allows you to specify the distribution of the data (family), which in this case is Poisson and specific to count data.  
```{r}
...
ggplot(regen_chi_squared, aes(x = Regen_count)) +
  geom_histogram()
```
Again, could be the zero inflation issue here as well. 
...
ggplot(total_regen, aes(x = Forest, y = total_regen)) +
  geom_bar(stat = 'identity', position = 'dodge') + #plotting the count 
  theme_bw() 
```
...
#### 7. Publication plot
```{r, echo=FALSE}
#Before plotting I am going to summarize the data a bit to get standard deviation for errorbars.
regen_for_plotting <- regen_chi_squared %>% group_by(Forest) %>%
  summarize(total_count = sum(Regen_count), sd_regen = sd(Regen_count))
```

```{r, echo=FALSE}
ggplot(regen_for_plotting, aes(x = Forest, y = total_count)) +
  geom_bar(stat = "identity") +
  geom_errorbar(aes(ymin= total_count-sd_regen, ymax= total_count+sd_regen), width=.2, position = position_dodge(.9)) +
  theme_bw() +
  ylab("Regen Count")
```
